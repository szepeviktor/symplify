...
use Symplify\PHPStanRules\Types\ValueObject\ClassMethodTypeAnalyzer;
...
final class NoProtectedElementInFinalClassRule extends AbstractSymplifyRule
{
    ...
    /**
     * @var \Symplify\PHPStanRules\Types\ValueObject\ClassMethodTypeAnalyzer
     */
    private $classMethodTypeAnalyzer;
    public function __construct(ParentMethodAnalyser $parentMethodAnalyser, ClassMethodTypeAnalyzer $classMethodTypeAnalyzer)
    {
...
CODE_SAMPLE
    ...
    private function processClassMethod(ClassMethod $classMethod, Class_ $class, Scope $scope): array
    {
        ...
        if ($this->doesMethodExistInTraits($class, $methodName) || $this->parentMethodAnalyser->hasParentClassMethodWithSameName($scope, $methodName)) {
            return [];
        }
    ...
    {
        return $this->classMethodTypeAnalyzer->isClassMethodOfNamesAndType($classMethod, $scope, ['configureRoutes', 'configureContainer'], MicroKernelTrait::class);
    }
}
