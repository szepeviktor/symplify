...
final class CheckConstantExpressionDefinedInConstructOrSetupRule extends AbstractSymplifyRule
{
    ...
    public function __construct(\PhpParser\NodeFinder $nodeFinder, SimpleNameResolver $simpleNameResolver, \PhpParser\ConstExprEvaluator $constExprEvaluator)
    {
...
CODE_SAMPLE
    ...
    private function isUsedInNextStatement(\PhpParser\Node\Expr\Assign $assign, Node $node): bool
    {
        $var = $assign->var;
    ...
    private function isHasSameVar(array $nodes, Node $parentOfParentAssignment, Node $var): bool
    {
        foreach ($nodes as $node) {
            $parentOfParentNode = $node->getAttribute(PHPStanAttributeKey::PARENT)->getAttribute(PHPStanAttributeKey::PARENT);
            if (property_exists($node, 'name') && property_exists($var, 'name') && $node->name === $var->name && $parentOfParentNode !== $parentOfParentAssignment) {
                return true;
            }
        }
        return false;
    }
    private function isInInstatiationClassMethod(\PhpParser\Node\Expr\Assign $assign): bool
    {
