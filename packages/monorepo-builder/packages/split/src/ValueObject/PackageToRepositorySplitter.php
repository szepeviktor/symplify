...
use Symplify\MonorepoBuilder\Split\ValueObject\PackageToRepositorySplitException;
...
final class PackageToRepositorySplitter
{
     ...
     */
    private $gitManager;
    public function __construct(\Symfony\Component\Console\Style\SymfonyStyle $symfonyStyle, FileSystemGuard $fileSystemGuard, \Symplify\MonorepoBuilder\Split\Process\ProcessFactory $processFactory, GitManager $gitManager)
    {
     ...
     * @throws \Symplify\MonorepoBuilder\Split\ValueObject\PackageToRepositorySplitException
     * @throws \Symplify\SmartFileSystem\ValueObject\DirectoryNotFoundException
     */
    public function splitDirectoriesToRepositories(array $splitConfig, string $rootDirectory, ?string $branch = null, ?int $maxProcesses = null, ?string $tag = null): void
    {
        if ($tag === null) {
            ...
            if ($this->gitManager->doTagsHaveCommitterDate()) {
                $message = sprintf('Some of the tags on this repository do not have committer date. This may lead to unwanted tag selection during split. You may want to use the "%s" parameter.', Option::TAG);
