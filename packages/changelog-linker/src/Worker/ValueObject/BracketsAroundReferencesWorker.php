...
namespace Symplify\ChangelogLinker\Worker\ValueObject;
...
final class BracketsAroundReferencesWorker implements WorkerInterface
{
     ...
     */
    private const CLOSES_KEYWORDS = ['close', 'closes', 'closed', 'fix', 'fixes', 'fixed', 'resolve', 'resolves', 'resolved'];
    public function processContent(string $content): string
    {
        // issue or PR references
        $content = Strings::replace($content, '#' . RegexPattern::PR_OR_ISSUE_NOT_IN_BRACKETS . '#', function (array $match): string {
    ...
    {
        return Strings::replace($content, sprintf('#(%s) \#%s#', implode('|', self::CLOSES_KEYWORDS), RegexPattern::VERSION_REGEX), '$1 [#$2]');
    }
}
