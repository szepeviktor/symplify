...
namespace Symplify\CodingStandard\Fixer\Annotation\ValueObject;
...
use Symplify\CodingStandard\TokenRunner\Analyzer\FixerAnalyzer\ValueObject\DoctrineBlockFinder;
...
final class DoctrineAnnotationNewlineInNestedAnnotationFixer extends AbstractDoctrineAnnotationFixer implements DocumentedRuleInterface
{
    ...
    /**
     * @var \Symplify\CodingStandard\TokenRunner\Analyzer\FixerAnalyzer\ValueObject\DoctrineBlockFinder
     */
...
CODE_SAMPLE
    ...
    protected function fixAnnotations(Tokens $tokens): void
    {
        ...
        for ($index = 0; $index < $tokenCount; ++$index) {
            ...
            $tokens->insertAt($index, new \PhpCsFixer\Doctrine\Annotation\Token(DocLexer::T_NONE, ' * '));
            $tokens->insertAt($index, new \PhpCsFixer\Doctrine\Annotation\Token(DocLexer::T_NONE, "\n"));
    ...
    private function processEndBracket(int $index, Tokens $tokens, int $previousTokenPosition): void
    {
        ...
        if ($this->currentBlockInfo !== null) {
            $tokens->insertAt($this->currentBlockInfo->getEnd(), new \PhpCsFixer\Doctrine\Annotation\Token(DocLexer::T_NONE, ' * '));
            $tokens->insertAt($this->currentBlockInfo->getEnd(), new \PhpCsFixer\Doctrine\Annotation\Token(DocLexer::T_NONE, "\n"));
        }
    }
    private function shouldSkip(int $index, Tokens $tokens, \PhpCsFixer\Doctrine\Annotation\Token $previousToken): bool
    {
